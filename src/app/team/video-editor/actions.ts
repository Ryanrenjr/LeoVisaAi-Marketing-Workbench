...
import { buildVideoImageSearchQueries } from "@/lib/ai/image-search-queries";
 ...
 * already-generated 视频号 script, downloads and re-hosts them into the
...
export async function searchAndAttachImages(topicId: string): Promise<{ ok: boolean; error?: string }> {
  ...
  if (!script) return { ok: false, error: "请先生成视频号文案，再搜索配图。" };

  const content = script.structured_content as unknown as VideoChannelContent;
  const queries = buildVideoImageSearchQueries(content);
