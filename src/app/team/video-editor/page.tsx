...
function TopicRow({
  ...
  const images = script ? (imagesByAssetId.get(script.id) ?? []) : [];
  return (
    <li className="card flex flex-col gap-2 px-5 py-4">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[var(--muted)]">
        <span>研究：已确认</span>
        <span>{platformStatusLabel(assets, "VIDEO_CHANNEL")}</span>
        <span>图片：{images.length > 0 ? "已配图" : "待配图"}</span>
      </div>
      <ContentImageGrid images={images} />
      {canRun && modelOptions && (
        <GenerateAction
          action={regeneratePlatformContent.bind(null, topic.id, "VIDEO_CHANNEL")}
          label={script ? "重新生成口播文案" : "生成口播文案"}
          variant={script ? "secondary" : "primary"}
...
export default async function VideoEditorPage() {
  const employee = getEmployee("video-editor");
  ...
  const employeeName = resolveEmployeeDisplayName("video-editor", employeeNames);
  ...
  const modelOptions = canRun ? await getTaskModelOptions("VIDEO_WRITING") : null;
